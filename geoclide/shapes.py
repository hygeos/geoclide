...
class Shape:
    ...
    def __init__(self, object_to_world: Transform, world_to_object: Transform):
        if not isinstance(object_to_world, Transform) or not isinstance(
            world_to_object, Transform
        ):
            raise ValueError(
                "The parameters object_to_world and world_to_object "
                "must be both Transform"
            )
        self.otw = object_to_world
        self.wto = world_to_object


def get_intersect_dataset(
    shape_name: str,
    r: Ray,
    ...
    dpdu: np.ndarray | None = None,
    dpdv: np.ndarray | None = None,
    diag_calc: bool = False,
) -> xr.Dataset:
    """
    Create dataset containing the intersection test information

    - The intersect method return (when parameter ds_output = False) of
      all quadrics, Triangle and TriangleMesh classes gives directly
    ...
    if dpdu is None:
        dpdu = np.array([np.nan, np.nan, np.nan])
    if dpdv is None:
    ...
    if not isinstance(t, np.ndarray):
        ...
        if t is None:
            ...
            n = face_forward(
                Normal(normalize(cross(Vector(dpdu), Vector(dpdv)))),
                -r.d,
            )
            n = n.to_numpy()
        ...
        n = face_forward(
            Normal(normalize(cross(Vector(dpdu), Vector(dpdv)))), -r.d
        )
        n = n.to_numpy()
        n[not_int] = None
    ...
    # multiple shape and rays but calc only the diagonal
    elif diag_calc or shape_name == "TriangleMesh":
        ndiag = len(t)
        ...
        if nobj >= nrays:
            ...
            for ir in range(0, nrays):
                ri = Ray(
                    Point(ro[ir, :]), Vector(rd[ir, :]), mint[ir], maxt[ir]
                )
            ...
            for iobj in range(0, nobj):
                n_bis = Normal(
                    normalize(
                        cross(Vector(dpdu[iobj, :]), Vector(dpdv[iobj, :]))
                    )
                )
    ...
    ds["u"].attrs = {
        "description": "the u coordinate of the parametric representation"
    }
    ds["v"].attrs = {
        "description": "the v coordinate of the parametric representation"
    }
    ds["phit"].attrs = {
        "type": "Point",
    ...
    ds["nhit"].attrs = {
        "type": "Normal",
        "description": "the x, y and z components of the normal at the "
        "intersection point",
    }
    ds["dpdu"].attrs = {
        "type": "Vector",
        "description": "the surface partial derivative of phit with "
        "respect to u",
    }
    ds["dpdv"].attrs = {
        "type": "Vector",
