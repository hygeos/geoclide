...
Q1 = [
    gc.Disk(radius=3.0, inner_radius=1.2),
    gc.Sphere(radius=3.0),
    gc.Spheroid(radius_xy=3.0, radius_z=2.0),
    gc.Spheroid(radius_xy=2.0, radius_z=3.0),
]
...
def test_sphere():
    ...
    vaa = 45.0
    sat_altitude = 700.0
    origin = gc.Point(0.0, 0.0, 0.0)
    theta = vza
    phi = -vaa

    dir_to_sat = gc.Vector(0.0, 0.0, 1.0)
    ...
    earth_radius = 6378.0
    otw = gc.get_translate_tf(gc.Vector(0.0, 0.0, -earth_radius))
    sphere_sat_alti = gc.Sphere(
        radius=earth_radius + sat_altitude, otw=otw
    )  # apply otw to move the sphere center to earth center
    ds_sp = gc.calc_intersection(sphere_sat_alti, ray)

    p = ds_sp["phit"].values

    assert np.isclose(p[0], 472.61058011386376, 0.0, 1e-15)
    assert np.isclose(p[1], -472.61058011386365, 0.0, 1e-15)
    assert np.isclose(p[2], 668.3722921180423, 0.0, 1e-15)
    assert ds_sp["is_intersection"] == sphere_sat_alti.is_intersection(ray)


def test_spheroid():
    oblate = gc.Spheroid(radius_xy=3.0, radius_z=1.5)
    prolate = gc.Spheroid(radius_xy=1.5, radius_z=3.0)
    r1 = gc.Ray(o=gc.Point(2.5, 0.0, 10.0), d=(gc.Vector(0.0, 0.0, -1.0)))
    r2 = gc.Ray(o=gc.Point(10.0, 0.0, 2.5), d=(gc.Vector(-1.0, 0.0, 0.0)))

    ds = gc.calc_intersection(oblate, r1)
    p = ds["phit"].values
    assert p[0] == 2.5
    assert p[1] == 0.0
    assert np.isclose(p[2], 0.8291561975888655, 0.0, 1e-15)
    n = ds["nhit"].values
    assert np.isclose(n[0], 0.6019292654288356, 0.0, 1e-15)
    assert n[1] == 0.0
    assert np.isclose(n[2], 0.7985494095046983, 0.0, 1e-15)
    assert ds["is_intersection"].values == oblate.is_intersection(r1)

    ds = gc.calc_intersection(prolate, r2)
    p = ds["phit"].values
    assert np.isclose(p[0], 0.8291561975888655, 0.0, 1e-15)
    assert p[1] == 0.0
    assert p[2] == 2.5
    n = ds["nhit"].values
    assert np.isclose(n[0], 0.7985494095046906, 0.0, 1e-15)
    assert n[1] == 0.0
    assert np.isclose(n[2], 0.6019292654288461, 0.0, 1e-15)

    assert np.isclose(oblate.area(), 78.04694433010546, 0.0, 1e-15)
    assert np.isclose(prolate.area(), 48.326479487738396, 0.0, 1e-15)
...
def test_disk():
    ...
    ds = gc.calc_intersection(
        disk, gc.Ray(gc.Point(5.0, 0.0, 0.0), gc.Vector(-1.0, 0.0, 0.0))
    )
    assert not ds["is_intersection"]
    ds = gc.calc_intersection(
        disk,
        gc.Ray(gc.Point(0.0, 0.0, 10.0), gc.Vector(0.0, 0.0, -1.0), maxt=5.0),
    )
    assert not ds["is_intersection"]

    disk = gc.Disk(radius=1.0, phi_max=90.0)
    ds = gc.calc_intersection(
        disk, gc.Ray(gc.Point(0.8, -1e-5, 10.0), gc.Vector(0.0, 0.0, -1.0))
    )
    assert not ds["is_intersection"]
    ds = gc.calc_intersection(
        disk, gc.Ray(gc.Point(0.8, 1e-5, 10.0), gc.Vector(0.0, 0.0, -1.0))
    ...
    ds = gc.calc_intersection(
        disk, gc.Ray(gc.Point(-1e-5, 0.8, 10.0), gc.Vector(0.0, 0.0, -1.0))
    )
    assert not ds["is_intersection"]
    ds = gc.calc_intersection(
        disk, gc.Ray(gc.Point(+1e-5, 0.8, 10.0), gc.Vector(0.0, 0.0, -1.0))
    ...
    # 2) general cases
    roty_90 = gc.get_rotate_y_tf(90.0)
    disk = gc.Disk(radius=1.5, z_height=5.0, otw=roty_90)
    annulus = gc.Disk(radius=1.5, inner_radius=0.8, z_height=5.0, otw=roty_90)
    d = gc.Vector(-1.0, 0.0, 0.0)
    r1 = gc.Ray(gc.Point(10.0, 0.0, 1.2), d)
    r2 = gc.Ray(gc.Point(10.0, 0.0, 0.2), d)
    r3 = gc.Ray(gc.Point(10.0, 0.0, 1.6), d)

    # 2a) disk tests
    ds_disk = gc.calc_intersection(disk, r1)
    p = ds_disk["phit"].values
    assert np.all(np.isclose(p, r1(disk.z_height).to_numpy(), 0.0, 1e-15))
    assert ds_disk["is_intersection"].values == disk.is_intersection(r1)
    ds_disk = gc.calc_intersection(disk, r2)
    p = ds_disk["phit"].values
    assert np.all(np.isclose(p, r2(disk.z_height).to_numpy(), 0.0, 1e-15))
    ...
    assert np.all(np.isclose(p, r1(annulus.z_height).to_numpy(), 0.0, 1e-15))
...
def test_quadric_1d_arr(quadric):
    ...
    assert np.array_equal(
        ds["is_intersection"].values, is_int_1d, equal_nan=True
    )
    assert np.array_equal(ds["thit"].values, t_1d, equal_nan=True)
    assert np.array_equal(ds["phit"].values, p_1d, equal_nan=True)
    assert np.allclose(ds["nhit"].values, n_1d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds["u"].values, u_1d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds["v"].values, v_1d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds["dpdu"].values, dpdu_1d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds["dpdv"].values, dpdv_1d, 0.0, 1e-14, equal_nan=True)
