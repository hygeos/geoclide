...
PHIS = np.linspace(0.0, 360.0, 6)
THETAS = np.linspace(-180.0, 180.0, 7)
...
def test_vec2ang(phi, theta):
    ...
    assert np.isclose(v1.x, v2.x, 0.0, 1e-14)
    assert np.isclose(v1.y, v2.y, 0.0, 1e-14)
    assert np.isclose(v1.z, v2.z, 0.0, 1e-14)
    ...
    assert np.isclose(v1.y, v2.y, 0.0, 1e-14)
    assert np.isclose(v1.z, v2.z, 0.0, 1e-14)
    assert np.isclose(v1.x, v3.x, 0.0, 1e-14)
    assert np.isclose(v1.y, v3.y, 0.0, 1e-14)
    assert np.isclose(v1.z, v3.z, 0.0, 1e-14)
...
def test_vec2ang_old_bug():
    ...
    gc.vec2ang(
        gc.Vector(
            -0.028830092518596997, 0.004805015419766166, -0.9995727775365759
        )
    )


def test_ang2vec_diag():
    theta = np.array([20.0, 30.0, 45.0])
    phi = np.array([0.0, 0.0, 180.0])
...
def test_vec2ang_arr():
    phis = np.linspace(0.0, 360.0, 6)
    thetas = np.linspace(-180.0, 180.0, 6)
    v_set1 = gc.ang2vec(
        theta=thetas, phi=phis, vec_view="zenith", diag_calc=True
    )
    ths, phs = gc.vec2ang(v_set1, vec_view="zenith")
    v_set2 = gc.ang2vec(theta=ths, phi=phs, vec_view="zenith", diag_calc=True)

    assert np.allclose(v_set1.to_numpy(), v_set2.to_numpy(), 0.0, 1e-15)

    v_set1 = gc.ang2vec(
        theta=thetas, phi=phis, vec_view="nadir", diag_calc=True
    )
