...
def test_triangle_intersection():
    p0 = gc.Point(-1.0, -1.0, 0.0)
    p1 = gc.Point(1.0, -1.0, 0.0)
    p2 = gc.Point(1.0, 1.0, 0.0)

    tri = gc.Triangle(p0, p1, p2)
    assert tri.area() == 2.0

    ray = gc.Ray(
        o=gc.Point(0.0, 0.0, 1.0),
        d=gc.normalize(gc.Vector(0.999, 0.999, -1.0)),
    )
    ...
    assert np.isclose(1.73089629960896, ds_v2["thit"].item(), 0.0, 1e-14), (
        "Problem with v2 intersection test"
    )
    assert np.isclose(1.73089629960896, ds_v3["thit"].item(), 0.0, 1e-14), (
        "Problem with v3 intersection test"
    )
    assert gc.Normal(ds_v2["nhit"].values) == gc.Normal(0.0, 0.0, 1.0), (
        "Problem with v2 intersection test"
    )
    assert gc.Normal(ds_v3["nhit"].values) == gc.Normal(0.0, 0.0, 1.0), (
        "Problem with v3 intersection test"
    )

    p3 = gc.Point(0.999, 0.999, 0.0)

    assert np.isclose(ds_v2["phit"].values[0], p3.x, 0.0, 1e-15), (
        "Problem with v2 triangle intersection test"
    )
    assert np.isclose(ds_v2["phit"].values[1], p3.y, 0.0, 1e-15), (
        "Problem with v2 triangle intersection test"
    )
    assert np.isclose(ds_v2["phit"].values[2], p3.z, 0.0, 1e-15), (
        "Problem with v2 triangle intersection test"
    )
    assert np.isclose(ds_v3["phit"].values[0], p3.x, 0.0, 1e-15), (
        "Problem with v3 triangle intersection test"
    )
    assert np.isclose(ds_v3["phit"].values[1], p3.y, 0.0, 1e-15), (
        "Problem with v3 triangle intersection test"
    )
    assert np.isclose(ds_v3["phit"].values[2], p3.z, 0.0, 1e-15), (
        "Problem with v3 triangle intersection test"
    )
    ...
    assert ds_v2["is_intersection"].item() is False, (
        "Problem with v2 intersection test"
    )
    assert ds_v3["is_intersection"].item() is False, (
        "Problem with v3 intersection test"
    )


def test_triangle_transform():
    p0 = gc.Point(-0.5, 0.5, 0.0)
    p1 = gc.Point(0.5, 0.5, 0.0)
    p2 = gc.Point(0.5, -0.5, 0.0)

    otw = gc.get_translate_tf(gc.Vector(10.0, 0.0, 5.0)) * gc.get_rotate_y_tf(
        45.0
    )
    ...
    ray = gc.Ray(
        o=gc.Point(0.0, 0.0, 4.8), d=gc.normalize(gc.Vector(1.0, 0.0, 0.0))
    )
    ...
    assert ds_v2["is_intersection"].item(), "Problem with v2 intersection test"
    assert ds_v3["is_intersection"].item(), "Problem with v3 intersection test"
    assert np.isclose(10.2, ds_v2["thit"].item(), 0.0, 1e-14), (
        "Problem with v2 intersection test"
    )
    assert np.isclose(10.2, ds_v3["thit"].item(), 0.0, 1e-14), (
        "Problem with v3 intersection test"
    )
    assert np.isclose(
        -math.sqrt(2.0) / 2.0, ds_v2["nhit"].values[0], 0.0, 1e-13
    ), "Problem with v2 intersection test"
    assert ds_v2["nhit"].values[1] == 0.0, "Problem with v2 intersection test"
    assert np.isclose(
        -math.sqrt(2.0) / 2.0, ds_v2["nhit"].values[2], 0.0, 1e-13
    ), "Problem with v2 intersection test"
    assert np.isclose(
        -math.sqrt(2.0) / 2.0, ds_v3["nhit"].values[0], 0.0, 1e-13
    ), "Problem with v3 intersection test"
    assert ds_v3["nhit"].values[1] == 0.0, "Problem with v3 intersection test"
    assert np.isclose(
        -math.sqrt(2.0) / 2.0, ds_v3["nhit"].values[2], 0.0, 1e-13
    ), "Problem with v3 intersection test"

    p3 = gc.Point(10.2, 0.0, 4.8)
    ...
    assert tri.is_intersection(ray, method="v2"), (
        "Problem with v2 is_intersection test"
    )
    assert tri.is_intersection(ray, method="v3"), (
        "Problem with v3 is_intersection test"
    )
...
def test_triangle_mesh():
    ...
    faces = np.array(
        [
            [0, 1, 2],  # vertices index of T0
            [2, 3, 1],
        ],
        dtype=np.int32,
    )  # vertices index of T1

    otw = gc.get_translate_tf(gc.Vector(10.0, 0.0, 5.0)) * gc.get_rotate_y_tf(
        45.0
    )
    tri_mesh = gc.TriangleMesh(vertices=vertices, faces=faces, otw=otw)
    assert tri_mesh.area() == 1.0
    ...
    assert np.isclose(10.2, thitv2, 0.0, 1e-14), (
        "Problem with v2 intersection test"
    )
    assert np.isclose(10.2, thitv3, 0.0, 1e-14), (
        "Problem with v3 intersection test"
    )
    assert np.isclose(-math.sqrt(2.0) / 2.0, nhitv2.x, 0.0, 1e-13), (
        "Problem with v2 intersection test"
    )
    assert nhitv2.y == 0.0, "Problem with v2 intersection test"
    assert np.isclose(-math.sqrt(2.0) / 2.0, nhitv2.z, 0.0, 1e-13), (
        "Problem with v2 intersection test"
    )
    assert np.isclose(-math.sqrt(2.0) / 2.0, nhitv3.x, 0.0, 1e-13), (
        "Problem with v3 intersection test"
    )
    assert nhitv3.y == 0.0, "Problem with v3 intersection test"
    assert np.isclose(-math.sqrt(2.0) / 2.0, nhitv3.z, 0.0, 1e-13), (
        "Problem with v3 intersection test"
    )
    ...
    assert np.isclose(phitv2.x, p3.x, 0.0, 1e-15), (
        "Problem with v2 triangle intersection test"
    )
    assert np.isclose(phitv2.y, p3.y, 0.0, 1e-15), (
        "Problem with v2 triangle intersection test"
    )
    assert np.isclose(phitv2.z, p3.z, 0.0, 1e-15), (
        "Problem with v2 triangle intersection test"
    )
    assert np.isclose(phitv3.x, p3.x, 0.0, 1e-15), (
        "Problem with v3 triangle intersection test"
    )
    assert np.isclose(phitv3.y, p3.y, 0.0, 1e-15), (
        "Problem with v3 triangle intersection test"
    )
    assert np.isclose(phitv3.z, p3.z, 0.0, 1e-15), (
        "Problem with v3 triangle intersection test"
    )

    assert tri_mesh.is_intersection(ray, method="v2"), (
        "Problem with v2 is_intersection test"
    )
    assert tri_mesh.is_intersection(ray, method="v3"), (
        "Problem with v3 is_intersection test"
    )


def test_read_gcnc_trianglemesh():
    msh_read = gc.read_trianglemesh(
        ROOTPATH + "/geoclide/tests/data/sphere_r1p5_resth18_resph36.gcnc"
    )
    msh = gc.Sphere(1.5).to_trianglemesh(reso_theta=18, reso_phi=36)
    assert np.all(np.isclose(msh_read.vertices, msh.vertices, 0.0, 1e-15))
    assert np.all(msh_read.faces == msh.faces)

    msh_read = gc.read_trianglemesh(
        ROOTPATH
        + "/geoclide/tests/data/sphere_r1p5_resth18_resph18_phimax180.gcnc"
    )
    msh = gc.Sphere(1.5, phi_max=180.0).to_trianglemesh(
        reso_theta=18, reso_phi=18
    )
    assert np.all(np.isclose(msh_read.vertices, msh.vertices, 0.0, 1e-15))
    assert np.all(msh_read.faces == msh.faces)
    ...
    msh_read = gc.read_trianglemesh(
        ROOTPATH
        + "/geoclide/tests/data/oblate_rxy1p5_rz1p2_resth18_resph36.gcnc"
    )
    ...
    msh_read = gc.read_trianglemesh(
        ROOTPATH
        + "/geoclide/tests/data/prolate_rxy1p5_rz3_resth18_resph36.gcnc"
    )
    msh = gc.Spheroid(1.5, 3.0).to_trianglemesh(reso_theta=18, reso_phi=36)
    assert np.all(np.isclose(msh_read.vertices, msh.vertices, 0.0, 1e-15))
    assert np.all(msh_read.faces == msh.faces)

    msh_read = gc.read_trianglemesh(
        ROOTPATH + "/geoclide/tests/data/disk_r1_reso36.gcnc"
    )
    msh = gc.Disk(1.0).to_trianglemesh(reso=36)
    assert np.all(np.isclose(msh_read.vertices, msh.vertices, 0.0, 1e-15))
    assert np.all(msh_read.faces == msh.faces)

    msh_read = gc.read_trianglemesh(
        ROOTPATH + "/geoclide/tests/data/disk_r1_zh5_reso36.gcnc"
    )
    msh = gc.Disk(1.0, z_height=5.0).to_trianglemesh(reso=36)
    assert np.all(np.isclose(msh_read.vertices, msh.vertices, 0.0, 1e-15))
    assert np.all(msh_read.faces == msh.faces)

    msh_read = gc.read_trianglemesh(
        ROOTPATH + "/geoclide/tests/data/annulus_r1_ir0p5_reso36.gcnc"
    )
    msh = gc.Disk(1.0, inner_radius=0.5).to_trianglemesh(reso=36)
    assert np.all(np.isclose(msh_read.vertices, msh.vertices, 0.0, 1e-15))
    assert np.all(msh_read.faces == msh.faces)

    msh_read = gc.read_trianglemesh(
        ROOTPATH
        + "/geoclide/tests/data/annulus_partial_r1_ir0p5_reso27_phimax270.gcnc"
    )
    msh = gc.Disk(1.0, inner_radius=0.5, phi_max=270).to_trianglemesh(reso=27)
    assert np.all(np.isclose(msh_read.vertices, msh.vertices, 0.0, 1e-15))
    assert np.all(msh_read.faces == msh.faces)
...
def test_trianglemesh_to_dataset():
    ...
    assert np.all(
        np.isclose(msh1.vertices, msh2["vertices"].values[0, :, :], 0.0, 1e-15)
    )
    assert np.all(msh1.faces == msh2["faces"].values[0, :, :])


def test_trianglemesh_numpy():
    msh = gc.Spheroid(radius_xy=1.0, radius_z=3).to_trianglemesh(
        reso_theta=45, reso_phi=90
    )
    r0 = gc.Ray(gc.Point(5.0, -0.2, 2.5), gc.Vector(-1.0, 0.0, 0.0))
    ...
    assert np.isclose(ds_v2["thit"].item(), ds_v2f["thit"].item(), 0.0, 1e-15)
    assert np.isclose(ds_v2["thit"].item(), ds_v3f["thit"].item(), 0.0, 1e-15)
    assert np.isclose(
        ds_v2["phit"].values[0], ds_v2f["phit"].values[0], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["phit"].values[1], ds_v2f["phit"].values[1], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["phit"].values[2], ds_v2f["phit"].values[2], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["phit"].values[0], ds_v3f["phit"].values[0], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["phit"].values[1], ds_v3f["phit"].values[1], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["phit"].values[2], ds_v3f["phit"].values[2], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdu"].values[0], ds_v2f["dpdu"].values[0], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdu"].values[1], ds_v2f["dpdu"].values[1], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdu"].values[2], ds_v2f["dpdu"].values[2], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdu"].values[0], ds_v3f["dpdu"].values[0], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdu"].values[1], ds_v3f["dpdu"].values[1], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdu"].values[2], ds_v3f["dpdu"].values[2], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdv"].values[0], ds_v2f["dpdv"].values[0], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdv"].values[1], ds_v2f["dpdv"].values[1], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdv"].values[2], ds_v2f["dpdv"].values[2], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdv"].values[0], ds_v3f["dpdv"].values[0], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdv"].values[1], ds_v3f["dpdv"].values[1], 0.0, 1e-15
    )
    assert np.isclose(
        ds_v2["dpdv"].values[2], ds_v3f["dpdv"].values[2], 0.0, 1e-15
    )
    assert np.isclose(ds_v2["u"].item(), ds_v2f["u"].item(), 0.0, 1e-15)
    assert np.isclose(ds_v2["u"].item(), ds_v3f["u"].item(), 0.0, 1e-15)
    assert np.isclose(ds_v2["v"].item(), ds_v2f["v"].item(), 0.0, 1e-15)
    assert np.isclose(ds_v2["v"].item(), ds_v3f["v"].item(), 0.0, 1e-15)
...
def test_triangle_2d_arr1():
    ...
    assert np.array_equal(
        ds_v3["is_intersection"].values, is_int_v3, equal_nan=True
    )
    ...
    for ir in range(0, nrays):
        list_rays.append(
            gc.Ray(gc.Point(o_set_arr[ir, :]), gc.Vector(d_set_arr[ir, :]))
        )
    ...
    assert np.allclose(ds_v3["thit"].values, t_2d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds_v3["phit"].values, p_2d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds_v3["nhit"].values, n_2d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds_v3["u"].values, u_2d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(ds_v3["v"].values, v_2d, 0.0, 1e-15, equal_nan=True)
    assert np.allclose(
        ds_v3["dpdu"].values, dpdu_2d, 0.0, 1e-15, equal_nan=True
    )
    assert np.allclose(
        ds_v3["dpdv"].values, dpdv_2d, 0.0, 1e-15, equal_nan=True
    )
    ...
    assert np.array_equal(
        msh_ds["thit"].values, msh_dsn["thit"].values, equal_nan=True
    )
    assert np.array_equal(
        msh_ds["phit"].values, msh_dsn["phit"].values, equal_nan=True
    )
    assert np.array_equal(
        msh_ds["nhit"].values, msh_dsn["nhit"].values, equal_nan=True
    )
    assert np.array_equal(
        msh_ds["u"].values, msh_dsn["u"].values, equal_nan=True
    )
    assert np.array_equal(
        msh_ds["v"].values, msh_dsn["v"].values, equal_nan=True
    )
    assert np.array_equal(
        msh_ds["dpdu"].values, msh_dsn["dpdu"].values, equal_nan=True
    )
    assert np.array_equal(
        msh_ds["dpdv"].values, msh_dsn["dpdv"].values, equal_nan=True
    )
    ...
    dimy = 2
    msh = gc.Sphere(
        4, otw=gc.get_translate_tf(gc.Vector(0.0, 3.5, 10.0))
    ).to_trianglemesh(9, 18)
    nx = 11
    ny = 11
    n_samples = nx * ny
    ...
    assert np.array_equal(
        ds["thit"].values, dsl["thit"].values, equal_nan=True
    )
    assert np.array_equal(
        ds["phit"].values, dsl["phit"].values, equal_nan=True
    )
    assert np.array_equal(
        ds["nhit"].values, dsl["nhit"].values, equal_nan=True
    )
    ...
    assert np.array_equal(
        ds["dpdu"].values, dsl["dpdu"].values, equal_nan=True
    )
    assert np.array_equal(
        ds["dpdv"].values, dsl["dpdv"].values, equal_nan=True
    )
...
def test_triangle_2d_arr2():
    ...
    # In v3 -> more robust test! even 1e-17 instead of 0. can lead to a
    # failed test
    ds_v3 = triangles.intersect_v3(r_set, diag_calc=False)
...
def test_triangle_1d_arr1():
    msh = gc.Sphere(1.0).to_trianglemesh(reso_theta=5, reso_phi=5)
    o = gc.Point(-0.4, 0.4, 2.0)
    d = gc.Vector(0.0, 0.0, -1.0)
    r0 = gc.Ray(o, d)
...
def test_triangle_1d_arr2():
    ...
    p0 = gc.Point(np.array([0.0, 0.0, 1]))
    p1 = gc.Point(np.array([0.70710678, 0.0, 0.70710678]))
    ...
    # Particular case where we should get the same values bellow
    # (because only 1 triangle)
    assert np.array_equal(
        ds_v3["is_intersection"].values,
        msh_ds["is_intersection"].values,
        equal_nan=True,
    )
    assert np.array_equal(
        ds_v3["thit"].values, msh_ds["thit"].values, equal_nan=True
    )
