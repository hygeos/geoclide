...
class Sphere(Shape):
    ...
    - z0, z1 and phi_max are needed parameters for the creation of any
      partial sphere
    ...
    z_min : float, optional
        The minimum z value of the sphere where z0 is between [-radius,
        0]
    z_max : float, optional
        The maximum z value of the sphere where z1 is between [0,
        radius]
    phi_max : float, optional
        The maximum phi value in degrees of the sphere, where phi is
        between 0 and 360°
    ...
    def __init__(
        ...
        if phi_max < 0.0 or phi_max > 360.0:
            raise ValueError(
                "The value of the parameter phi_max must in the "
                "range: [0, 360]"
            )
    ...
    def is_intersection_t(
        ...
        if is_r_arr:
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                c6_bis_1 = np.logical_and(
                    self.zmin > -self.radius, phit.z < self.zmin
                )
                c6_bis_2 = np.logical_and(
                    self.zmax < self.radius, phit.z > self.zmax
                )
                c6_bis_3 = phi > phi_max_rad
                c6 = np.logical_and(
                    c4, np.logical_or.reduce((c6_bis_1, c6_bis_2, c6_bis_3))
                )
    ...
    def is_intersection(self, r: Ray) -> bool | np.ndarray:
        """
        Test if a ray/set of rays intersects the sphere / partial sphere
    ...
    def intersect(self, r: Ray, ds_output: bool = True) -> xr.Dataset | tuple:
        """
        Test if a ray/set of rays intersects the sphere/partial sphere
        ...
        * xyz              (xyz) int64 24B 0 1 2
        ...
        if is_r_arr:
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                dpdu = Vector(
                    -phi_max_rad * phit.y, phi_max_rad * phit.x, zeros
                )
                ...
                out = (
                    ...
                    u,
                    v,
                    self.otw(dpdu).to_numpy(),
                    self.otw(dpdv).to_numpy(),
                    False,
                )
                if ds_output:
    ...
    def to_trianglemesh(
        self, reso_theta: int | None = None, reso_phi: int | None = None
    ) -> TriangleMesh:
        """
        ...
        theta_zmax = clamp(math.degrees(self.theta_max), 0.0, 360.0)
        theta_min = min(theta_zmin, theta_zmax)
        theta_max = max(theta_zmin, theta_zmax)
        return create_sphere_trianglemesh(
            self.radius,
            reso_theta,
            reso_phi,
            theta_min,
    ...
    def plot(self, **kwargs):
        ...
        - The sphere is first converted to a triangle mesh then the
          TriangleMesh plot method is used

        Parameters
        ----------
        **kwargs
            The keyword arguments are passed on to the TriangleMesh plot
            method
        """
...
class Spheroid(Shape):
    ...
    radius_z : float
        The pole radius of the spheroid (distance from center to pole
        along z axis)
    otw : Transform, optional
        From object to world space or the transformation applied to the
        spheroid
        ...
        if is_r_arr:
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                a = (
                    ...
                    + ray.d.z * ray.d.z * inv_gamma2
                )
                b = 2 * (
                    ray.d.x * ray.o.x * inv_alpha2
                    + ray.d.y * ray.o.y * inv_beta2
                    + ray.d.z * ray.o.z * inv_gamma2
                )
                c = (
                    ray.o.x * ray.o.x * inv_alpha2
                    + ray.o.y * ray.o.y * inv_beta2
                    + ray.o.z * ray.o.z * inv_gamma2
                    - 1
                )
        ...
        if is_r_arr:
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                dpdv = Vector(
                    fac * cosphi,
                    fac * sinphi,
                    math.pi * self.gamma * np.sin(theta),
                )
            ...
            sinphi = phit.y * invzradius
            fac = -math.pi * (self.alpha / self.gamma) * phit.z
            dpdu = Vector(-TWO_PI * phit.y, TWO_PI * phit.x, 0.0)
            dpdv = Vector(
                fac * cosphi,
                fac * sinphi,
                math.pi * self.gamma * math.sin(theta),
            )
    ...
    def area(self) -> float:
        ...
        if self.gamma < self.alpha:  # oblate spheroid
            ...
            area = TWO_PI * self.alpha2 + math.pi * (
                self.gamma2 / e
            ) * math.log((1 + e) / (1 - e))
            ...
            area = (
                TWO_PI
                * self.alpha2
                * (1 + (self.gamma / (self.alpha * e)) * math.asin(e))
            )
        else:  # sphere
            area = 4.0 * math.pi * self.alpha2
        return area
    ...
    def plot(self, **kwargs):
        ...
        - The spheroid is first converted to a triangle mesh then the
          TriangleMesh plot method is used
...
class Disk(Shape):
    ...
    Notes
    -----
    Even if z_height is given, the origin for rotation transformation do
    not change. For exemple: z_height=5 then we apply a rotation of 90
    degrees around the y axis, the disk we be rotated from (0.,0.,0.),
    meaning the disk we be moved from position (0.,0.,5.) to (5.,0.,0.).
    """
        ...
        * xyz              (xyz) int64 24B 0 1 2
        ...
        if is_r_arr:
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                dpdv = Vector(phit.x, phit.y, zeros) * (
            ...
            dpdu = Vector(-phi_max_rad * phit.y, phi_max_rad * phit.x, 0.0)
            dpdv = Vector(phit.x, phit.y, 0.0) * (
                (self.inner_radius - self.radius) / hit_radius
            )
    ...
    def to_trianglemesh(self, reso: int | None = None) -> TriangleMesh:
        """
        ...
        ----------
        reso : int, optional
            The number of lines around the polar phi angle, minimum
            accepted value is 3
    ...
    def plot(self, **kwargs):
        ...
        - The disk is first converted to a triangle mesh then the
          TriangleMesh plot method is used
