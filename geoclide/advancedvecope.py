...
def ang2vec(
    ...
    """
    Convert a direction/directions described by 2 angles/set of 2
    angles into a direction/directions described by a vector/vectors
    ...
    Notes
    -----
    In case both theta and phi are 1-D ndarrays, the vectors are
    ordered in phi-major order
    ...
    if vec_view == "zenith":
        ...
        raise ValueError(
            "The value of vec_view parameter must be: 'zenith' or 'nadir'"
        )
...
def vec2ang(
    v: Vector, vec_view: str = "zenith", acc: float = 1e-6
    ...
    if isinstance(v.x, np.ndarray):
        ...
        c1 = np.all(np.isclose(v_arr, v_ini_arr, 0.0, acc), axis=1)
        ...
        for icase in range(1, 6):
            if icase == 1:
                ...
                if v.x == 0 and roty_rad == 0:
                    cosphi = 0.0
                else:
                    cosphi = clamp(v.x / math.sin(roty_rad), -1.0, 1.0)
            ...
            if np.all(
                np.isclose(
                    v.to_numpy() - v_ini_rotated.to_numpy(), 0.0, 0.0, acc
                )
            ):
                break
