...
class Triangle(Shape):
    ...
    otw : Transform, optional
        From object to world space or the transformation applied to the
        triangle
    wto : Transform, optional
        From world to object space or the in inverse transformation
        applied to the triangle
    p0t : Point, optional
        If given circumvent the automatically computed p0t (p0 after
        applying transformation)
    p1t : Point, optional
        If given circumvent the automatically computed p1t (p1 after
        applying transformation)
    p2t : Point, optional
        If given circumvent the automatically computed p2t (p2 after
        applying transformation)
    """
    ...
    def __init__(
        ...
        if (
            not isinstance(p0, Point)
            or not isinstance(p1, Point)
            or not isinstance(p2, Point)
        ):
            raise ValueError("The parameters p0, p1 and p2 must be all Point")
        if (
            (p0t is not None and not isinstance(p0t, Point))
            or (p1t is not None and not isinstance(p1t, Point))
            or (p2t is not None and not isinstance(p2t, Point))
        ):
            raise ValueError(
    ...
    def is_intersection(
        self, r: Ray, method: str = "v3", diag_calc: bool = False
    ) -> bool | np.ndarray:
        """
        ...
        diag_calc : bool, optional
            ...
            ndarray point components, meaning the output is a 1-D array
    ...
    def is_intersection_t(
        self, r: Ray, method: str = "v3", diag_calc: bool = False
    ) -> tuple[float | np.ndarray | None, bool | np.ndarray]:
        """
        Test if a ray/set of rays intersect with the triangle(s)
    ...
    def is_intersection_v2_t(
        self, r: Ray, diag_calc: bool = False
    ) -> tuple[float | np.ndarray | None, bool | np.ndarray]:
        """
        :meta private:

        Test if a ray/set of rays intersect with the triangle(s) using
        mainly pbrt v2 method
        ...
        if is_p_arr and is_r_arr and not diag_calc:
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                if ntriangles >= nrays:
                    ...
                    # Compute triangle partial derivatives Below the z
                    # components is not needed since we are in 2D with u
                    # in x and v un y
                    dpdu = Vector()
                    dpdv = Vector()
                    uv0 = Point(0.0, 0.0, 0.0)
                    uv1 = Point(1.0, 0.0, 0.0)
                    uv2 = Point(1.0, 1.0, 0.0)
                    duv02 = uv0 - uv2
                    duv12 = uv1 - uv2
                    ...
                    c5_bis_1 = np.logical_or(
                        degenerate, gv.cross(dpdu, dpdv).length_squared() == 0
                    )
                    ...
                    for ir in range(0, nrays):
                        ...
                        c3_bis_1 = np.logical_and(
                            det < 0,
                            np.logical_or(
                                t_scaled >= 0, t_scaled < r.maxt * det
                            ),
                        )
                        c3_bis_2 = np.logical_and(
                            det > 0,
                            np.logical_or(
                                t_scaled <= 0, t_scaled > r.maxt * det
                            ),
                        ...
                        # Ensure that computed triangle t is
                        # conservatively greater than zero
                        max_zt = np.max(
                            np.abs(np.array([p0t.z, p1t.z, p2t.z])), axis=0
                        )
                        delta_z = GAMMA3_F64 * max_zt
                        max_xt = np.max(
                            np.abs(np.array([p0t.x, p1t.x, p2t.x])), axis=0
                        )
                        max_yt = np.max(
                            np.abs(np.array([p0t.y, p1t.y, p2t.y])), axis=0
                        )
                        delta_x = GAMMA5_F64 * (max_xt + max_zt)
                        delta_y = GAMMA5_F64 * (max_yt + max_zt)
                        delta_e = 2 * (
                            GAMMA2_F64 * max_xt * max_yt
                            + delta_y * max_xt
                            + delta_x * max_yt
                        )
                        max_e = np.max(np.abs(np.array([e0, e1, e2])), axis=0)
                        delta_t = (
                            3
                            * (
                                GAMMA3_F64 * max_e * max_zt
                                + delta_e * max_zt
                                + delta_z * max_e
                            )
                            * abs(inv_det)
                    ...
                    for itri in range(0, ntriangles):
                        ...
                        # Compute scaled hit distance to triangle and
                        ...
                        # Compute triangle partial derivatives Below the
                        # z components is not needed since we are in 2D
                        # with u in x and v un y
                        dp02 = p0 - p2
                        dp12 = p1 - p2
                        ...
                        if not degenerate:
                            invdet = 1.0 / determinant
                            dpdu_bis = (
                                duv12.y * dp02 - duv02.y * dp12
                            ) * invdet
                            dpdv_bis = (
                                -duv12.x * dp02 + duv02.x * dp12
                            ) * invdet

                        ng = gv.cross(p2 - p0, p1 - p0)
                        c5_bis_1 = (
                            degenerate
                            or gv.cross(dpdu_bis, dpdv_bis).length_squared()
                            == 0
                        )
                        c5_bis_2 = ng.length_squared() == 0
            ...
            # Get triangle vertices and translate them in based on ray
            # origin
            p0t = p0 - r.o
            p1t = p1 - r.o
            p2t = p2 - r.o
            ...
            if is_p_arr or is_r_arr:
                with np.errstate(divide="ignore", invalid="ignore"):
                    ...
                    c3_bis_1 = np.logical_and(
                        det < 0,
                        np.logical_or(t_scaled >= 0, t_scaled < r.maxt * det),
                    )
                    c3_bis_2 = np.logical_and(
                        det > 0,
                        np.logical_or(t_scaled <= 0, t_scaled > r.maxt * det),
                    ...
                    # Compute barycentric coordinates and t value for
                    # triangle intersection
                    inv_det = 1.0 / det
                    t = t_scaled * inv_det

                    # Ensure that computed triangle t is conservatively
                    # greater than zero
                    max_zt = np.max(np.abs(np.array([p0t.z, p1t.z, p2t.z])))
                    delta_z = GAMMA3_F64 * max_zt
                    max_xt = np.max(np.abs(np.array([p0t.x, p1t.x, p2t.x])))
                    max_yt = np.max(np.abs(np.array([p0t.y, p1t.y, p2t.y])))
                    delta_x = GAMMA5_F64 * (max_xt + max_zt)
                    delta_y = GAMMA5_F64 * (max_yt + max_zt)
                    delta_e = 2 * (
                        GAMMA2_F64 * max_xt * max_yt
                    ...
                    max_e = np.max(np.abs(np.array([e0, e1, e2])))
                    delta_t = (
                        3
                        * (
                            GAMMA3_F64 * max_e * max_zt
                ...
                if (
                    det < 0 and (t_scaled >= 0 or t_scaled < r.maxt * det)
                ) or (det > 0 and (t_scaled <= 0 or t_scaled > r.maxt * det)):
                    return None, False
                ...
                # Compute triangle partial derivatives Below the z
                # components is not needed since we are in 2D with u in
                # x and v un y
                dpdu = Vector()
                dpdv = Vector()
                uv0 = Point(0.0, 0.0, 0.0)
    ...
    def is_intersection_v3(self, r: Ray, diag_calc: bool) -> bool | np.ndarray:
        """
        :meta private:
    ...
    def intersect(
        ...
        """
        Test if a ray/set of rays intersect with the triangle(s) and
        ...
        Notes
        -----
        By default the 'v3' method is used since there are more
        robustness tests. But the 'v2' method is at least twice faster
        than 'v3'.
        """
        if method == "v3":
            return self.intersect_v3(
                r, diag_calc=diag_calc, ds_output=ds_output
            )
        elif method == "v2":
            return self.intersect_v2(
                r, diag_calc=diag_calc, ds_output=ds_output
            )
        else:
            raise ValueError(
    ...
    def intersect_v2(
        ...
        Test if a ray/set of rays intersect with the triangle(s) using
        mainly pbrt v2 method, and return intersection information
        ...
        if is_p_arr and is_r_arr and not diag_calc:
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                if ntriangles >= nrays:
                    ...
                    uvs = np.array([[0.0, 0.0], [1.0, 0.0], [1.0, 1.0]])
                    ...
                    if determinant == 0:
                        dpdu, dpdv = gv.coordinate_system(
                            gv.normalize(gv.cross(e2, e1))
                        )
                    else:
                        invdet = 1.0 / determinant
                    ...
                    for ir in range(0, nrays):
                        ...
                        # interpolate $(u,v)$ triangle parametric
                        # coordinates
                        b0 = 1 - b1 - b2
                    ...
                    if ds_output:
                        return get_intersect_dataset(*out)
                    else:
                        return out
                else:  # nrays > npoints
                    ray = Ray(r)
                    ...
                    for itri in range(0, ntriangles):
                        ...
                        if determinant == 0:
                            dpdu_bis, dpdv_bis = gv.coordinate_system(
                                gv.normalize(gv.cross(e2, e1))
                            )
                        else:
                            invdet = 1.0 / determinant
    ...
    def intersect_v3(
        ...
        Test if a ray/set of rays intersect with the triangle(s) using
        mainly pbrt v3 method, and return intersection information
    ...
    def area(self) -> float | np.ndarray:
        ...
        .. warning::
            `the scale transformation is not considered for the area
            calculation!`
        """
...
class TriangleMesh(Shape):
    ...
    def __init__(
        ...
        if wto is None and otw is None:
            wto = Transform()
            otw = Transform()
        elif wto is None and isinstance(otw, Transform):
            wto = otw.inverse()
        elif isinstance(wto, Transform) and otw is None:
            otw = wto.inverse()
        if (
            not isinstance(faces, np.ndarray)
            or not (len(faces.shape) == 2)
            or not (
                np.issubdtype(faces.dtype, int)
                or np.issubdtype(faces.dtype, np.integer)
            )
        ):
            raise ValueError(
                "The parameter faces must be a 2d ndarray of intergers"
            )
        if not (isinstance(vertices, np.ndarray)) or not (
    ...
    def intersect(
        ...
        """
        Test if a ray/set of rays intersect with the triangle mesh and
        return intersection information
        ...
        if use_loop:
            if nrays > 1 and not diag_calc:
                ...
                for ir in range(0, nrays):
                    ...
                    thit = float("inf")
                    for itri in range(0, self.ntriangles):
                        res_bis = triangles[itri].intersect(
                            ri, method=method, ds_output=False
                        )
                        thit_bis = res_bis[2]
                ...
                if ds_output:
                    return get_intersect_dataset(*res_1d)
                else:
                    return res_1d
            elif diag_calc:
                is_int_1d = np.full((nrays), True, dtype=bool)
                ...
                for idiag in range(0, ndiag):
                    ...
                    thit = float("inf")
                    ri = Ray(
                        Point(o_set_arr[idiag, :]), Vector(d_set_arr[idiag, :])
                    )
                ...
                for itri in range(0, self.ntriangles):
                    ...
                    triangle = Triangle(p0, p1, p2, self.otw, self.wto)
                    res_bis = triangle.intersect(
                        r, method=method, ds_output=False
                    )
                    thit_bis = res_bis[2]
                    is_intersection = res_bis[3]
                    if is_intersection and thit > thit_bis:
            ...
            res_bis = triangles.intersect(
                r, method=method, diag_calc=diag_calc, ds_output=False
            )
        ...
        if use_loop:
            ...
            if nrays > 1 and not diag_calc:
                ...
                for idiag in range(0, ndiag):
                    ...
                    p2 = Point(self.vertices[self.faces[idiag, 2], :])
                    triangle = Triangle(p0, p1, p2, self.otw, self.wto)
                    is_int_1d[idiag] = triangle.is_intersection(
                        ri, method=method
                    )
                return is_int_1d
    ...
    def is_intersection_t(
        ...
        Notes
        -----
        If use_loop = True, is_intersection_t can be significantly more
        consuming than is_intersection. Because it does not stop at the
        first intersection, but it finalize the complete loop to return
        the thit corresponding to the nearest triangle.
        """
        if not isinstance(r, Ray):
            raise ValueError("The parameter r must be a Ray")
        if isinstance(r.o.x, np.ndarray):
    ...
    def plot(
        ...
        **kwargs,
        ...
        ----------
        source : str
            The package used for the plot, only 2 option -> 'matplotlib'
            or 'trimesh'. If source = None, use matplolib for mesh with
            ntriangle < 5000, else use trimesh
        savefig_name : str, optional
            If savefig_name is given, the figure is saved with the given
            name (only if source='matplotlib)
        **kwargs
            All other keyword arguments are passed on to matplotlib
            plot_trisurf function. For example: alpha, color, shade, ...
            If source = 'trimesh' then the keyword arguments passed on
            to show Trimesh method
        ...
        if self.otw.is_identity():
            ...
            vertices = self.otw(Point(self.vertices)).to_numpy()
        ...
        if (
            ...
            raise ValueError(
                "Unknown source. Please choose between: 'matplotlib' "
                "or 'trimesh'"
            )

    def to_dataset(self, name: str = "none") -> xr.Dataset:
        """
        Create an xarray dataset where the triangle mesh information are
        stored
        ...
        -------
        Dataset
            Xarray dataset containing the triangle mesh
            information.

            Key variables included:

            - **obj_names**: The name(s) of the mesh(es)
            - **vertices**: The vertices xyz coordinates
              [nobj, nvertices, xyz]
            - **faces**: For each triangle, the indices of its
              vertices p0, p1 and p2 [nobj, ntriangles, p0p1p2]
        """
        ...
        ds["vertices"] = xr.DataArray(
            vertices, dims=["nboj", "nvertices", "xyz"]
        )
        ds["vertices"].attrs = {"description": "The vertices xyz coordinates."}
        ds["faces"] = xr.DataArray(
            faces, dims=["nobj", "ntriangles", "p0p1p2"]
        )
        ds["faces"].attrs = {
    ...
    def write(self, path: str, **kwargs) -> None:
        ...
        **kwargs
            The keyword arguments are passed on to xarray to_netcdf or
            trimesh export method
        """
...
def create_sphere_trianglemesh(
    radius: float,
    reso_theta: int | None = None,
    reso_phi: int | None = None,
    theta_min: float = 0.0,
    ...
    Create a sphere / partial sphere triangle_mesh
    ...
    reso_theta : int, optional
        The number of lines around the polar theta angle, minimum
        accepted value is 3
    reso_phi : int, optional
        The number of lines around the azimuth phi angle, minimum
        accepted value is 3
    ...
    if wto is None and otw is None:
        wto = Transform()
        otw = Transform()
    if reso_theta is None:
        reso_theta = max(round(theta_max / 10.0), 10)
    if reso_phi is None:
        reso_phi = max(round(theta_max / 10.0), 10)
    if reso_theta < 3:
        raise ValueError("the value of reso_theta must >= 3")
    if reso_phi < 3:
        raise ValueError("the value of reso_phi must >= 3")
    if phi_max == 360.0:
        phi_max = math.tau
    else:
        phi_max = math.radians(phi_max)
    ...
    if phi_max < math.tau:
        phi = np.linspace(0.0, phi_max, reso_phi)
    else:
        phi = np.linspace(0.0, phi_max, reso_phi + 1)
    ...
    if theta[0] == 0.0 and theta[-1] == math.pi:
        nvertices = reso_theta * reso_phi - (reso_phi * 2) + 2
        ntriangle = (nvertices - 2) * 2
        if phi[-1] < math.tau:
            ntriangle -= 2
    elif theta[0] == 0.0 or theta[-1] == math.pi:
    ...
    if phi[-1] < math.tau:
        ntriangle -= max((reso_theta - 3) * 2, 0)
    ...
    for ith in range(0, reso_theta - 1):
        if theta[ith] == 0.0:
            ...
            if phi[-1] < math.tau:
                faces[0 : reso_phi - 1, 0] = np.full(reso_phi, ind_above)[
                    :-1
                ]  # p0
                ...
                ini_id += reso_phi
        elif theta[ith] > 0.0 and theta[ith + 1] < math.pi:
            # case with polygones of 4 vertices, then split in 2 to get
            # triangles
            if ini_id == 0:
                p0_id = 0
            else:
                p0_id = np.max(faces) + 1 - reso_phi
            ...
            if phi[-1] < math.tau:
                ...
                p0_id_t = np.zeros(
                    (p0_id_t0.size + p0_id_t1.size), dtype=np.int32
                )
                p1_id_t = np.zeros(
                    (p1_id_t0.size + p1_id_t1.size), dtype=np.int32
                )
                p2_id_t = np.zeros(
                    (p2_id_t0.size + p2_id_t1.size), dtype=np.int32
                )
...
def create_disk_trianglemesh(
    ...
    Create a disk / partial disk / annulus / partial annulus
    triangle_mesh
    ...
    reso : int, optional
        The number of lines around the polar phi angle, minimum accepted
        value is 3
    phi_max : float, optional
        The maximum phi value in degrees of the disk/annulus, where phi
    ...
    if wto is None and otw is None:
        ...
        otw = Transform()
    if reso is None:
        reso = max(round(phi_max / 10.0), 10)
    if reso < 3:
        raise ValueError("the value of reso must >= 3")

    if phi_max == 360.0:
        phi = np.linspace(0.0, math.tau, reso + 1)[:-1]
    else:
        phi = np.linspace(0.0, math.radians(phi_max), reso)
    ...
    if inner_radius == 0.0:
        ...
        if phi_max == 360.0:
            ...
            p2_id = np.concatenate(
                (vertices_id_outer[1:], [vertices_id_outer[0]])
            )
        else:
        ...
        if phi_max == 360.0:
            ...
            p2_id_t0 = np.concatenate(
                (vertices_id_outer[1:], [vertices_id_outer[0]])
            )
            p0_id_t1 = vertices_id_inner
            p1_id_t1 = np.concatenate(
                (vertices_id_outer[1:], [vertices_id_outer[0]])
            )
            p2_id_t1 = np.concatenate(
                (vertices_id_inner[1:], [vertices_id_inner[0]])
            )
        else:
...
def read_gcnc_trianglemesh(path: str, **kwargs) -> TriangleMesh:
    """
    Read geoclide netcdf4 format and convert it to a TriangleMesh class
    object
    ...
    **kwargs
        The keyword arguments are passed on to xarray open_dataset
        method
    ...
    if "filename_or_obj" in kwargs:
        kwargs.pop("filename_or_obj", False)
    if not path.endswith("gcnc"):
        raise ValueError(
            "Only the geoclide netcdf4 format (ending with 'gcnc') is accepted"
        )
    if "engine" in kwargs:
        engine = kwargs.pop("engine", False)
    else:
        engine = "netcdf4"
    ds = xr.open_dataset(path, engine=engine, **kwargs)
    return TriangleMesh(
        ds["vertices"].values[0, :, :], ds["faces"].values[0, :, :]
    )


def read_trianglemesh(path: str, **kwargs) -> TriangleMesh:
    """
    Open mesh file
