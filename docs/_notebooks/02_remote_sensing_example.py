...
#
# Knowing its altitude and its viewing zenith and azimuth angles.
...
# origin is the viewer seeing the satellite
origin = gc.Point(0., 0., 0.)
# The vaa start from north going clockwise.
# Let's assume that in our coordinate system the x axis is in the
# north direction
...
# create the ray, starting from origin going in dir_to_sat direction
...
# %%
b1 = gc.BBox(p1=gc.Point(-math.inf, -math.inf, 0.),
             p2=gc.Point(math.inf, math.inf, sat_altitude))
...
sphere_sat_alti = gc.Sphere(radius=earth_radius+sat_altitude, otw=otw)
...
y = y.flatten()
z = np.zeros_like(x)
# id = (0, 0, 0) corresponds to the pixel at the center of the camera
id_pixels = gc.Point(x,y,z)
...
# satellite position/pixel positions. We duplicate to get same size
...
(dir_pixels.to_numpy()[is_intersection][0:10],
 id_pixels.to_numpy()[is_intersection][0:10])
